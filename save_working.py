# it can dowlaond multiple links in the form of a zip file
...
def download_youtube_video(link, output_path):
    try:
        ...
        if duration <= 120:
            ...
            st.write(f"Video '{yt.title}' has been downloaded successfully.")
            return True
        else:
    ...
    try:
        ...
        if st.button("Download Selected Videos"):
            for link in selected_video_links:
                success = download_youtube_video(link, output_folder)
                if not success:
